  ...
  for (Layout &layout : solid.data.layout) {
    ...
    case Topology::Line:
      ...
      for (size_t i = 0; i < layout.count; ++i) {
        ...
        size_t &index_b = solid.data.indices[layout.start + (i * 2) + 1];
    ...
    case Topology::Triangle:
      ...
      for (size_t i = 0; i < layout.count; ++i) {
        ...
        size_t &index_b = solid.data.indices[layout.start + (i * 3) + 1];
        size_t &index_c = solid.data.indices[layout.start + (i * 3) + 2];
...
CpuRenderer::CpuRenderer(SceneInfo &scene_info) : m_scene_info_ref(scene_info) {
  ...
  pipeline_triangle_data.matrix_calculation = Pipeline::matrix_calculation_smvp;
  ...
  pipeline_triangle_data.clip_after_dehomog = Pipeline::clip_after_dehomog_none;
  pipeline_triangle_data.trasform_vertices_onto_viewport =
      Pipeline::trasform_vertices_onto_viewport;
  pipeline_triangle_data.rasterization = Pipeline::rasterization_triangle_fill;
  ...
  auto &pipeline_simulated_line_data = scene_info.pipeline_simulated_line_data;
  ...
  pipeline_simulated_line_data.trasform_vertices_onto_viewport =
      Pipeline::trasform_vertices_onto_viewport;
  pipeline_simulated_line_data.rasterization = Pipeline::rasterization_line_dda;
  pipeline_simulated_line_data.set_pixel =
      std::bind(&CpuRenderer::set_pixel_rgba_depth, this, std::placeholders::_1,
                std::placeholders::_2, std::placeholders::_3);
  ...
  pipeline_simulated_triangle_data.set_pixel =
      std::bind(&CpuRenderer::set_pixel_rgba_depth, this, std::placeholders::_1,
                std::placeholders::_2, std::placeholders::_3);
...
void CpuRenderer::update_pipeline_settings() {
  ...
  case SetPixelEnum::BW_DEPTH:
    m_scene_info_ref.pipeline_simulated_triangle_data.set_pixel =
        std::bind(&CpuRenderer::set_pixel_bw_depth, this, std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3);
    break;
  }
}
...
CpuRenderer::render_simulation_with_callback() {
  ...
  for (Layout &layout : solid.data.layout) {
    ...
    case Topology::Triangle:
      ...
      pipeline_triangle_data.proj_matrix =
          m_scene_info_ref.simulated_camera->get_projection_matrix();
      pipeline_triangle_data.trasform_vertices_onto_viewport = [](auto, auto,
                                                                  auto) {};
      ...
      for (size_t i = 0; i < layout.count; ++i) {
        ...
        if (callback) {
          ...
          data.matrix = {glm::inverse(pipeline_triangle_data.proj_matrix *
                                      pipeline_triangle_data.view_matrix)};
